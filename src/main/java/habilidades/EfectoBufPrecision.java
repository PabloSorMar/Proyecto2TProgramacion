...
/**
 * Efecto mecánico: Aumenta la precisión de un aliado aleatorio (autobuf
 * generalmente).
 */
public class EfectoBufPrecision extends Habilidades {
     ...
     */
    public EfectoBufPrecision(String nombre, int cooldownBase, String efecto, boolean distancia, String tipo,
            int aumento) {
    ...
    public void EjecutarHabilidad(List<Entidad> objetivos) {
        if (super.estaDisponible()) {
            ...
            System.out.println("  La habilidad " + this.getNombre() + " aumenta la puntería de " + objetivo.getNombre()
                    + " (+" + aumentoPrecision + ")");
