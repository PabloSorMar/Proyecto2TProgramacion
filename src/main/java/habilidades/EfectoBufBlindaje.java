...
public class EfectoBufBlindaje extends Habilidades {
     ...
     */
    public EfectoBufBlindaje(String nombre, int cooldownBase, String efecto, boolean distancia, String tipo,
            int aumento) {
    ...
    public void EjecutarHabilidad(List<Entidad> objetivos) {
        if (super.estaDisponible()) {
            for (Entidad objetivo : objetivos) {
                System.out.println(
                        "  La habilidad " + this.getNombre() + " aumenta el blindaje de " + objetivo.getNombre());
