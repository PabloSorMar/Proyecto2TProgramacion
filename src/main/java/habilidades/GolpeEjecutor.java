...
/**
 * Golpe ejecutor: elimina al instante a un objetivo si su vida es baja (menor o
 * igual a 60).
        ...
        if (super.estaDisponible()) {
            ...
            if (ran.nextInt(0, 100) < this.getPrecision()) {
                ...
                if (objetivo.getVida() <= 60) {
                    System.out
                            .println("  La habilidad " + this.getNombre() + " EJECUTA a " + objetivo.getNombre() + "!");
                    objetivo.setVida(0);
                } else {
                    System.out.println("  La habilidad " + this.getNombre() + " golpea a " + objetivo.getNombre()
                            + " (vida demasiado alta para ejecutar, 35 de dano)");
